...
{
	function GetTemplates($relativePath, $wizardRelativePath)
	{
		$absolutePath = $_SERVER["DOCUMENT_ROOT"].$relativePath;
		$absolutePath = str_replace("\\", "/", $absolutePath);

		$wizardAbsolutePath = $_SERVER["DOCUMENT_ROOT"].$wizardRelativePath;
		$wizardAbsolutePath = str_replace("\\", "/", $wizardAbsolutePath);
		...
		while(($dirName = @readdir($handle)) !== false)
		{
			...
			if (LANGUAGE_ID != "ru" && file_exists($wizardAbsolutePath."/images/templates/".$dirName."/screen_".LANGUAGE_ID.".gif"))
				$arTemplate["SCREENSHOT"] = $wizardRelativePath."/images/templates/".$dirName."/screen_".LANGUAGE_ID.".gif";
			elseif (file_exists($wizardAbsolutePath."/images/templates/".$dirName."/screen.gif"))
				$arTemplate["SCREENSHOT"] = $wizardRelativePath."/images/templates/".$dirName."/screen.gif";
			else
				$arTemplate["SCREENSHOT"] = false;

			if (file_exists($wizardAbsolutePath."/images/templates/".$dirName."/preview.gif"))
				$arTemplate["PREVIEW"] = $wizardRelativePath."/images/templates/".$dirName."/preview.gif";
			else
	...
	function GetThemes($relativePath, $wizardRelativePath, $templateID)
	{
		$arThemes = Array();
		...
		if ($handle = @opendir($themePath))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				$arThemes[$file] = $arTemplate + Array(
					...
					"PREVIEW" => (file_exists($wizardAbsolutePath."/images/templates/".$templateID."/themes/".$file."/preview.gif") ? $wizardRelativePath."/images/templates/".$templateID."/themes/".$file."/preview.gif" : false),
					"SCREENSHOT" => (file_exists($wizardAbsolutePath."/images/templates/".$templateID."/themes/".$file."/screen_".LANGUAGE_ID.".gif") ? $wizardRelativePath."/images/templates/".$templateID."/themes/".$file."/screen.gif" : false),
				);
	...
	function AddMenuItem($menuFile, $menuItem, $siteID, $pos = -1)
	{
		...
		{
			$docRoot = CSite::GetSiteDocRoot($siteID);
			$arResult = CFileMan::GetMenuArray($docRoot.$menuFile);
