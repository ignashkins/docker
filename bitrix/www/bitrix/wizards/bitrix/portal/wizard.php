...
class PortalFeaturesSettingsStep extends CWizardStep
{
	...
	function InitStep()
	{
		...
		if (
			...
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."about/")
		)
		{
			unset($this->arBXFeaturesPrintable["Portal"]["company"]);
		}

		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."/services/links.php")
		)
		{
			unset($this->arBXFeaturesPrintable["Portal"]["services"]["WebLink"]);
		}

		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."/services/votes.php")
		)
		{
			unset($this->arBXFeaturesPrintable["Portal"]["services"]["Vote"]);
		}

		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."/services/subscr_edit.php")
		)
		{
			unset($this->arBXFeaturesPrintable["Portal"]["services"]["Subscribe"]);
		}

		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."/services/board/")
		)
		{
			unset($this->arBXFeaturesPrintable["Portal"]["services"]["Board"]);
		}

		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."/services/learning/")
		)
		{
			unset($this->arBXFeaturesPrintable["Portal"]["services"]["Learning"]);
		}

		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."/services/wiki.php")
		)
		{
			unset($this->arBXFeaturesPrintable["Portal"]["services"]["Wiki"]);
		}
	}
...
class CommunicationsFeaturesSettingsStep extends PortalFeaturesSettingsStep
{
	function InitStep()
	{
		...
		if (
			...
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."services/idea/")
		)
		{
			$key = array_search("Idea", $this->arBXFeaturesPrintable["Communications"]);
			unset($this->arBXFeaturesPrintable["Communications"][$key]);
		}

		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."services/requests/")
		)
		{
			$key = array_search("Requests", $this->arBXFeaturesPrintable["Communications"]);
			unset($this->arBXFeaturesPrintable["Communications"][$key]);
		}
	}
...
class EnterpriseFeaturesSettingsStep extends PortalFeaturesSettingsStep
{
	function InitStep()
	{
		...
		if (
			$_SERVER["PHP_SELF"] == "/index.php"
			|| !IsModuleInstalled("support")
			|| !file_exists($_SERVER["DOCUMENT_ROOT"].$siteDir."services/support.php")
		)
		{
			$key = array_search("Support", $this->arBXFeaturesPrintable["Enterprise"]);
			unset($this->arBXFeaturesPrintable["Enterprise"][$key]);
		}
	}
...
class SiteSettingsStep extends CWizardStep
{
	...
	{
		if (file_exists($filename) && ($siteLogo = file_get_contents($filename)) !== false && $siteLogo <> '')
		{
	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if (!$firstStep && $site_id != "s1" && !defined("ADDITIONAL_INSTALL"))
			{
				$siteFolder = $wizard->GetVar("siteFolderDep");

				if (trim($siteFolder, " /") == '' || !preg_match('#^/(\w+|_)/$#', $siteFolder))
				{
	...
	function ShowStep()
	{
		...
		else
		{
			...
			{
				if ($_SERVER["PHP_SELF"] == "/index.php" && !defined("ADDITIONAL_INSTALL"))
				{
...
class LDAPSettingsStep extends CWizardStep
{
	...
	{
		return (isset($_REQUEST["checkServer"]) && $_REQUEST["checkServer"] <> '');
	}
	...
	function OnPostForm()
	{
		...
		if($wizard->GetVar('ldapNTLM')=='Y' && $wizard->GetVar('ldapNTLMDomain') == '')
		{
	...
	function ShowStep()
	{
		...
		if(mb_strpos($_SERVER['SERVER_SOFTWARE'], 'mod_auth_sspi') !== false)
		{
...
class LDAPGroupsStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		elseif($wizard->GetVar('ldapNTLM')=='Y' && $wizard->GetVar('ldapNTLMDomain') <> '')
		{
...
class DataInstallStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		if ($_SERVER["PHP_SELF"] == "/index.php" && !NON_INTRANET_EDITION)
		{
			...
			{
				$ar = array("PersonalPhoto", "Blog", "Gallery");
				foreach ($ar as $f)
...
class FinishStep extends CWizardStep
{
	...
	function ShowStep()
	{
		...
		if(IsModuleInstalled("extranet")
			&&
			(
				($_SERVER["PHP_SELF"] == "/index.php" && CBXFeatures::IsFeatureEnabled("Extranet"))
				|| COption::GetOptionString("main", "wizard_install_extranet", "N", "") == "Y"
			)
		)
		{
...
//b24 to cp
		...
		COption::SetOptionString("main", "wizard_solution", $wizard->GetID(), false, $siteID);
	}
}
?>